...
const CartDrawer = ({ children, isOpen, onCartIconClick }: Props) => {
	return (
		<div className='relative'>
			<div
				className={`fixed top-0 right-0 h-full w-90 text-white transition duration-700 ease-in-out transform z-10 ${
			...
			>
				<aside className='h-full overflow-y-auto bg-white'>
					...
					<main className='mt-8 p-4 text-black'>{children}</main>
				</aside>
			</div>
		</div>
	)
